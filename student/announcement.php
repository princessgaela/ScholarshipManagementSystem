...
function get_all_records()
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $Sql = "SELECT * FROM announcements WHERE academicyear = '$year' AND semester LIKE '$semester' ORDER BY id DESC LIMIT 2";
        $result = mysqli_query($con, $Sql);
        if (mysqli_num_rows($result) > 1) {
            ...
            <div class="card text-center">
                ...
                <pclass="card-title">(' . $row['date'] . ')</p>
                <button type="button" id="' . $row['id'] . '" class="btn btn-success editbtn" > View Announcement </button>
                </div>
            </div>
        </div>';
            }
    ...
    <div class="col">
        ...
        <div class="card text-center">
            <div class="card-body">
                <h5 class="card-title">No Announcement</h5>
            </div>
        </div>
    </div>
    <div class="col">
...
function get_all_announcements()
{
    ...
    if (mysqli_num_rows($result) > 0) {
        ...
        $year = $row['year'];
        $semester = $row['semester'];
        $Sql = "SELECT * FROM announcements WHERE academicyear = '$year' AND semester LIKE '$semester' ORDER BY id DESC LIMIT 18446744073709551615 OFFSET 2";
        $result = mysqli_query($con, $Sql);
        if (mysqli_num_rows($result) > 0) {

            while ($row = mysqli_fetch_assoc($result)) {
                echo "<button type='button' id='" . $row['id'] . "' class='btn btn-link editbtn'>" . $row['subject'] . "</button>" . $row['date'] . "<br>";
            }
        } else {
...
<body>
    ...
    <div class="container-fluid">
        <div class="text-center">
            <h3 style="font-size:35px; margin-top:10px;">Announcements</h3>
        </div>
    </div>
        ...
        <div class="row row-cols-1 row-cols-md-3 g-4">
            ...
            <div class="col">
                <div class="card text-left">
                    <div class="card-body">
                        <h5 class="card-title">Other Announcements</h5>
                        <?php get_all_announcements() ?>
                    </div>
                </div>
            </div>
        </div><br>
